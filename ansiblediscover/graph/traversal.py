...
class Traversal:
    ...
    def all(nodes: Set[Node], succ: Callable[[Node], Set[Node]]):
        nodes_to_visit = set(successor for node in nodes for successor in succ(node))
        all_nodes = set()

        while len(nodes_to_visit) > 0:
            all_nodes.update(nodes_to_visit)
            nodes_to_visit = set(successor for node in nodes_to_visit for successor in succ(node))
    ...
    def leafs(nodes: Set[Node], succ: Callable[[Node], Set[Node]]):
        ...
        while len(nodes_to_visit) > 0:
            leafs = set(node for node in nodes_to_visit if len(succ(node)) == 0)
